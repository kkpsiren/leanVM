...
fn cache_footprint(first_pubkey: &XmssPublicKey) -> u128 {
    // 1 + 1 + MESSAGE_LEN_FE(4) + XMSS_DIGEST_LEN(2) = 8 elements fit one width-8 compression.
    ...
    let digest = poseidon8_compress(input);
    // Two 64-bit Goldilocks limbs -> 128-bit footprint.
    digest[..2]
        .iter()
        .fold(0u128, |acc, f| (acc << 64) | u128::from(f.as_canonical_u64()))
}
