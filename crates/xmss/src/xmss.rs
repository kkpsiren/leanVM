...
fn gen_wots_secret_key(seed: &[u8; 32], slot: u32, public_param: PublicParam) -> WotsSecretKey {
    ...
    // DIGEST_LEN_FE (= 4) Goldilocks elements, 8 bytes each, fill the 32-byte RNG seed.
    for (chunk, f) in rng_seed.chunks_exact_mut(8).zip(rng_seed_fe) {
        chunk.copy_from_slice(&f.as_canonical_u64().to_le_bytes());
    }
    ...
    for level in (from_level + 1)..=to_level {
        ...
        let nodes: Vec<Digest> = {
            ...
            fill(sequential, &mut nodes, |k, out| {
                ...
                *out = poseidon8_compress(merkle_data)[..XMSS_DIGEST_LEN].try_into().unwrap();
            });
            nodes
        };
        layers.push(nodes);
