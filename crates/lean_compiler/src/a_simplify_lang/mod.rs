...
pub fn simplify_program(mut program: Program) -> Result<SimpleProgram, String> {
    ...
    let unroll_counter = Counter::new();
    let inline_counter = Counter::new();
    compile_time_transform_in_program(&mut program, &unroll_counter, &inline_counter)?;
    ...
    let mutable_loop_counter = Counter::new();
    transform_mutable_in_loops_in_program(&mut program, &mutable_loop_counter);
...
fn compile_time_transform_in_lines(
    ...
    while i < lines.len() {
        ...
        // Match arms are independent transforms — for the SPHINCS pattern a
        // single 256-arm Match dominates main's body, so we recurse in
        // parallel and merge the per-arm `new_functions` afterwards. The
        // unroll/inline counters are atomic, so handing them out from
        // multiple threads is safe; resulting IDs are unique but their
        // assignment order is non-deterministic. That changes the textual
        // form of the generated bytecode (different `@inlined_var_N_…`
        // suffixes → different FP-relative offsets) but not the executed
        // operations, instruction count, or memory layout sizes — so runtime
        // perf of the produced bytecode is preserved.
        const PAR_MATCH_THRESHOLD: usize = 8;
        let arms_count = lines[i].nested_blocks_mut().len();
        if matches!(lines[i], Line::Match { .. }) && arms_count >= PAR_MATCH_THRESHOLD {
            let arm_locals: Vec<Result<BTreeMap<String, Function>, String>> = lines[i]
                .nested_blocks_mut()
                .into_par_iter()
                .map(|block| {
                    let mut local_new = BTreeMap::new();
                    compile_time_transform_in_lines(
                        block,
                        const_arrays,
                        existing_functions,
                ...
                .collect();
            for arm_local in arm_locals {
                for (name, f) in arm_local? {
                    new_functions.entry(name).or_insert(f);
                }
            }
        } else {
...
fn transform_mutable_in_loops_in_program(program: &mut Program, counter: &Counter) {
