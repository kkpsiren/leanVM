...
fn parse_array_literal(pair: ParsePair<'_>, ctx: &mut ParseContext, const_name: &str) -> ParseResult<ConstArrayValue> {
    let elements: Vec<ConstArrayValue> = pair
        .into_inner() // grammar invariant: yields only Rule::array_element pairs
        .map(|element_pair| {
...
impl VarOrConstantParser {
    fn parse_identifier_or_constant(text: &str, ctx: &ParseContext) -> ParseResult<SimpleExpr> {
        ...
        // A named constant or numeric literal resolves to a scalar; otherwise it's
        // a variable reference.
        match ctx.resolve_scalar_text(text) {
            Some(value) => Ok(SimpleExpr::scalar(value)),
            None => Ok(VarOrConstMallocAccess::Var(text.to_string()).into()),
        }
    }
}
    ...
    fn parse(&self, pair: ParsePair<'_>, ctx: &mut ParseContext) -> ParseResult<F> {
        ...
        match inner.as_rule() {
            Rule::constant_value => {
                ...
                ctx.resolve_scalar_text(text).ok_or_else(|| {
                    SemanticError::with_context(
