...
def decompose_and_verify_merkle_batch_with_height(
    ...
    if num_chunks == 16:
        ...
        return
    if num_chunks == 32:
        decompose_and_verify_merkle_batch_const(num_queries, sampled, root, height, 32, stir_points, answers)
        return
    if num_chunks == 8:
        decompose_and_verify_merkle_batch_const(num_queries, sampled, root, height, 8, stir_points, answers)
        return
    if num_chunks == 12:
        decompose_and_verify_merkle_batch_const(num_queries, sampled, root, height, 12, stir_points, answers)
        return
    if num_chunks == 20:
        decompose_and_verify_merkle_batch_const(num_queries, sampled, root, height, 20, stir_points, answers)
        return
    if num_chunks == 24:
        decompose_and_verify_merkle_batch_const(num_queries, sampled, root, height, 24, stir_points, answers)
        return
    if num_chunks == 2:
        decompose_and_verify_merkle_batch_const(num_queries, sampled, root, height, 2, stir_points, answers)
        return
    if num_chunks == 4:
