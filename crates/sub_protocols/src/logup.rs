...
pub fn prove_generic_logup(
    ...
    bytecode_acc: &[F],
    traces: &BTreeMap<Table, TableMatrix>,
    ...
    for (table, _) in &tables_log_heights_sorted {
        ...
        while bus_idx < buses.len() {
            if next_group < mem_groups.len() && mem_groups[next_group].start_bus == bus_idx {
                ...
                let col_index = trace.column(group.idx_col);
                ...
                denominators[offset / width..][..group_len * packed_chunk_size]
                    ...
                    .for_each(|(i, denom_chunk)| {
                        ...
                        let col_value = trace.column(group.value_cols[i]);
            ...
            match bus.multiplicity {
                ...
                BusMultiplicity::Column(col) => {
                    fill_num_from(slice, trace.column(col), matches!(bus.direction, BusDirection::Pull));
                }
            }
            ...
            let ds_col: Option<&[F]> = match bus.domainsep {
                BusData::Column(c) => Some(trace.column(c)),
                _ => None,
            };
    ...
    for table in ALL_TABLES {
        ...
        let resolve_ef = |entry: BusData| -> EF {
            match entry {
                BusData::Column(col) => trace.column(col).evaluate(&inner_point),
                BusData::ColumnPlusConstant(col, ofs) => trace.column(col).evaluate(&inner_point) + F::from_usize(ofs),
            ...
            match bus.multiplicity {
                BusMultiplicity::Column(mult_col) => {
                    let eval_on_multiplicity =
                        trace.column(mult_col).evaluate(&inner_point) * bus.direction.to_field_flag();
                        ...
                        .filter_map(|entry| {
                            entry.column().and_then(|col| {
                                if let std::collections::btree_map::Entry::Vacant(e) = table_values.entry(col) {
                                    let v = trace.column(col).evaluate(&inner_point);
                                    e.insert(v);
                                    Some(v)
                                } else {
                                    None
